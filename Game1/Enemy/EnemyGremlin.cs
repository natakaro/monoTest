...
namespace Game1
{
    public class EnemyGremlin : EnemyWalk
    {

        public EnemyGremlin(Game game, Matrix inWorldMatrix, Model inModel, Octree octree, ItemManager itemManager, ContentManager Content, List<Vector3> path) : base(game, inWorldMatrix, inModel, octree, itemManager, Content, path)
        {
            animatedModel = new AnimatedModel("Models/gremlin");
            animatedModel.LoadContent(Content);

            walk = new AnimatedModel("Models/gremlin_walk");
            ...
            texture = GameplayScreen.assetContentContainer.enemyGremlinTexture;
