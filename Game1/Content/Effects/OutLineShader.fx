...
float4 PixelShaderr(float2 Tex: TEXCOORD0) : COLOR
{
	float4 Color = tex2D(ColorMapSampler, Tex);	
	float2 QuadScreenSize = float2(1920,1080);

	float2 ox = float2(Thickness,0.0);
    float2 oy = float2(0.0,Thickness);
    float2 uv = Tex.xy;
    float2 PP = uv - oy;
...
technique PostOutline
{
	pass P0
	{
		PixelShader = compile ps_4_0 PixelShaderr();
	}
}