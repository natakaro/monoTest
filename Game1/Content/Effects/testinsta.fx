...
float3 sunposition = float3(0, 0, 0);
 
//--------------------------- DIFFUSE LIGHT PROPERTIES ------------------------------
// The direction of the diffuse light
float3 DiffuseLightDirection = float3(1, 5, 0);
...
VertexToPixel CelVertexShader(AppToVertex input, float4x4 instanceTransform)
{
	...
	//DiffuseLightDirection = float3(sunposition[0] - instanceTransform[3][0], sunposition[1] - instanceTransform[3][1], sunposition[2] - instanceTransform[3][2]);
...
{
	float t = sunposition[0] - input.instanceTransform[3][0];
	float tt = sunposition[1] - input.instanceTransform[3][1];
	float ttt = sunposition[2] - input.instanceTransform[3][2];
	float3 temp = float3(t, tt, ttt);
	DiffuseLightDirection = temp;
	DiffuseIntensity = 5 - (length(temp) / 1000);
	//DiffuseIntensity = 5 - (length(temp) / 1000)
...
technique Toon
{
    ...
    pass Pass2
    {
        ...
        //PixelShader = compile ps_5_0 bla();
