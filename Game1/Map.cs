...
namespace Game1
{
    static class Map
    {
        ...
        public static List<DrawableObject> CreateMap(Game game, int size, Matrix inWorldMatrix, Model inModel)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                for (int j = 0; j < size; j++)
                {
                    ...
                    tileList.Add(new Tile(game, Matrix.CreateScale(scale) * Matrix.CreateTranslation(position) * inWorldMatrix, inModel));                   
                }
            }
            return tileList;
        }
